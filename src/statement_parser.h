...
struct statement{
	enum {
			s_case, s_label, s_default,	//labeled-statement
			s_compound,					//compound-statement
			s_expression,				//expression-statement
			s_if, s_if_else, s_switch,	//selection-statement
			s_for, s_while, s_do_while,	//iteration_statement
			s_goto, s_continue, s_break, s_return} t;
										//jump_statement
	union{
		struct block block;
		struct {
