...
#undef XX
	...
	memcpy(ret_val->full_type_specifiers,
	       ...
	       4);
	break;
    case t_s_double: case t_s_float:
	memcpy(ret_val->full_type_specifiers,
	       ret_val->core_type==t_s_float ? "f32" : "f64",
	       4);
	break;
    case t_s_char:
	memcpy(ret_val->full_type_specifiers,
...
static inline struct decl_type get_declarator(struct context* input) {
    ...
    case t_identifier:
	ret_val = (struct decl_type) { // base case...
	    .t = d_base,
	    .name = strdup(t->str),
	};
    ...
    case t_EOF: case t_int: case t_bad_token: case t_unknown: //fallthrough
    default:
	log_error("expected identifier, '[' or '(' before '%s'",
		  t->str);
	exit(1);
...
struct init_declaration_list* parse_declaration(struct context* input) {
    ...
    *ret_val = (struct init_declaration_list) {
	...
	if (ret_val->size++ % 4 == 0) { // realloc the declaration
	    ...
	    ret_val->vars = xrealloc(ret_val->vars,
				     s*sizeof(*ret_val->vars));
    ...
    free(specs);
    if (trailing_comma)
	log_error("trailing comma in declaration\n");
...
static inline void print_decl_type(struct decl_type* d_t, int indent) {
	...
	break;
    case d_base:
	break; // no point in printing the name
    }
}
...
void print_declaration(struct init_declaration_list* decls, int) {
	...
	// now print base type
	if (var->specifiers.is_const)
	    printf("const ");
	if (var->specifiers.is_restrict)
	    printf("restrict ");
	if (var->specifiers.is_volatile)
	    printf("volatile ");
	if (var->specifiers.is_inline)
	    printf("inline ");
