...
static inline int is_type(char* str) {
    char types[][8]
        = {"void",     "char",     "int",   "float",    "double",   "signed",
           "unsigned", "long",     "short", "typedef",  "extern",   "static",
           "auto",     "register", "const", "restrict", "volatile", "inline"};
...
struct statement* parse_statement(struct context* input) {
    ...
    if (strcmp(t->str, "if") == 0) {
        ...
        if (strcmp(peek(input)->str, "else")) s->t = s_if;
        else {
            next(input);
    ...
    if (strcmp(t->str, "switch") == 0) {
        ...
        while (strcmp(peek(input)->str, "}")
               != 0) { /*add proper EOF checking here*/ ///
            if (((s->block.size - 1) & s->block.size) == 0)
                s->block.e = xrealloc(s->block.e, sizeof(struct block_element)
                                                      * s->block.size);
...
void print_statement(struct statement* s, int indent) {
    ...
    case s_compound:;
        ...
        break;
    case s_expression:
        if (s->e != NULL) print_expr_ast(s->e, indent);
        else
            puts(";");
        break;
    case s_if:
    ...
    case s_do_while:
        ...
        print_expr_ast(s->e, indent + 1);
        break;
    case s_goto: printf("goto %s\n", s->label); break;
    case s_continue: puts("continue"); break;
    case s_break: puts("break"); break;
    case s_return:
        puts("return");
        if (s->e != NULL) print_expr_ast(s->e, indent + 1);
        break;
    default: printf("something really bad has happend!"); exit(1);
    }
}
